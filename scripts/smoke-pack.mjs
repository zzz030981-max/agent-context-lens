...
const runCommand = (command, args, options) => process.platform === "win32"
  ? execFileSync(process.env.ComSpec, ["/d", "/s", "/c", command, ...args], options)
  : execFileSync(command, args, options);
const npm = process.platform === "win32" ? "npm.cmd" : "npm";
const runNpm = (args, options) => runCommand(npm, args, options);
const output = runNpm(["pack", "-w", "agent-context-lens", "--json"], { cwd: root, encoding: "utf8" });
...
try {
  ...
  runCommand(bin, ["inspect", fixture, "--file", "src/auth/login.ts", "--agent", "codex"], { cwd: temp, stdio: "inherit" });
} finally {
