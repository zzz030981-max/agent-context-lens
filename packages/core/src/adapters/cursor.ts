...
import { exists, readRepositoryText } from "../utils.js";
    ...
    if (await exists(legacy)) {
      try {
        sources.push(makeSource({
          root: context.root, agent: "cursor", file: legacy, content: await readRepositoryText(context.root, legacy), priority: 5,
      ...
      } catch { blockedPaths.push(legacy); }
    }
